...
def crossref(doi: str) -> dict:
    ...
    if publication["status"] != "ok":
        raise requests.exceptions.HTTPError(
            f'Publication with DOI <{doi}> was not found! {publication["status"]}'
        )
    ...
    try:
        result["date_published"] = dateparser.parse(
            publication["created"]["date-time"]
        ).isoformat()
    except:
        pass
    ...
    try:
        ...
        for i, author in enumerate(publication["author"]):
            ...
            try:
                ...
                pass
            try:
                parsed_author["orcid"] = clean_orcid(author["ORCID"])
            except:
                pass
            try:
        ...
        try:
            for identifier in author["nameIdentifiers"]:
                if identifier["nameIdentifierScheme"].lower() == "orcid":
                    parsed_author["orcid"] = clean_orcid(identifier["nameIdentifier"])
        except:
            pass
        try:
            parsed_author["affiliations"] = author["affiliation"]["name"]
...
def zenodo(doi: str) -> dict:
    ...
    try:
        description = (
            lxml.html.fromstring(j["metadata"]["description"]).text_content().strip()
        )
    ...
    try:
        result["date_modified"] = datetime.datetime.fromisoformat(
            j["updated"]
        ).isoformat()
    except:
        pass
...
def get_author_affiliations(author: dict, orcid_token: str = None) -> typing.List[str]:
    """
    ...
    if "openalex" in author:
        ...
        try:
            if isinstance(author["openalex"], list):
                ...
                affiliations.append(
                    openalex.get_author_affiliations(author["openalex"])
                )
        except Exception as e:
            logger.debug(
                f"Could not get affiliation from openalex <{author['openalex']}>: {e}"
            )
    if "orcid" in author:
        orcid = ORCID(token=orcid_token)
        try:
            affiliations += orcid.get_author_affiliations(clean_orcid(author["orcid"]))
        except Exception as e:
            logger.debug(
                f"Could not get affiliation from orcid <{author['orcid']}>: {e}"
            )
    return list(set(affiliations))


def resolve_authors(
    authors_tmp: typing.List[dict], orcid_token: str = None
) -> typing.List[dict]:
    """
    ...
    for author in authors_tmp:
        if "orcid" in author:
            ...
            try:
                openalex_id = openalex.get_author_by_orcid(orcid_id)["id"].replace(
                    "https://openalex.org/", ""
                )
                try:
            ...
            try:
                orcid_details = orcid.resolve_author(
                ...
                if orcid_details:
                    ...
                    try:
                        openalex_id = openalex.get_author_by_orcid(
                            orcid_details["orcid-id"]
        ...
        if not "affiliations" in author:
            author["affiliations"] = get_author_affiliations(
                author, orcid_token=orcid_token
            )
